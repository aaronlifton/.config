local Util = require("util.lazy.core.util")
...
function M.hererocks()
  if M.options.rocks.hererocks == nil then M.options.rocks.hererocks = vim.fn.executable("luarocks") == 0 end
...
function M.setup(opts)
  ...
  if type(M.options.spec) == "string" then M.options.spec = { import = M.options.spec } end
  table.insert(M.options.install.colorscheme, "habamax")

  -- root
  M.options.root = Util.norm(M.options.root)
  if type(M.options.dev.path) == "string" then M.options.dev.path = Util.norm(M.options.dev.path) end
  ...
  if M.options.performance.reset_packpath then vim.go.packpath = vim.env.VIMRUNTIME end
  ...
  if M.headless() then
    ...
    vim.api.nvim_create_autocmd("User", {
      ...
      callback = function()
        require("lazy.view.commands").setup()
        if M.options.change_detection.enabled then require("lazy.manage.reloader").enable() end
        ...
        vim.api.nvim_create_autocmd("BufWritePost", {
          ...
          callback = function()
            ...
            if plugin then require("lazy").pkg({ plugins = { plugin } }) end
          end,
        })
