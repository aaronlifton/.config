...
function M.setup(opts)
  ...
  require("cmp.utils.snippet").parse = function(input)
    ...
    return Util.lazy.util.cmp.snippet_preview(input)
  end

  local cmp = require("cmp")
  cmp.setup(opts)
  cmp.event:on("confirm_done", function(event)
    if vim.tbl_contains(opts.auto_brackets or {}, vim.bo.filetype) then Util.lazy.util.cmp.auto_brackets(event.entry) end
  end)
  cmp.event:on("menu_opened", function(event)
    Util.lazy.util.cmp.add_missing_snippet_docs(event.window)
  end)
end
