...
function M.formatter(opts)
  ...
  local ret = {
    ...
    priority = 1,
    format = function(buf)
      M.format(Util.lazy.merge({}, filter, { bufnr = buf }))
    end,
    sources = function(buf)
      local clients = M.get_clients(Util.lazy.merge({}, filter, { bufnr = buf }))
      ---@param client vim.lsp.Client
  ...
  return Util.lazy.merge(ret, opts) --[[@as LazyFormatter]]
end
...
function M.format(opts)
  opts = vim.tbl_deep_extend(
    ...
    {},
    opts or {},
    Util.lazy.opts("nvim-lspconfig").format or {},
    Util.lazy.opts("conform.nvim").format or {}
  )
