...
function M.formatexpr()
  if Util.lazy.has("conform.nvim") then return require("conform").formatexpr() end
...
function M.format(opts)
  ...
  if not done and opts and opts.force then Util.lazy.warn("No formatter available", { title = "LazyVim" }) end
end
...
function M.health()
  ...
  if has_plugin and not has_extra then
    Util.lazy.warn({
      "`conform.nvim` and `nvim-lint` are now the default formatters and linters in Util.lazy.",
      "",
...
function M.snacks_toggle(buf)
  return {
    map = function(self, lhs)
      if not _G.Snacks then
        -- Fallback to regular keymap if Snacks isn't available
        vim.keymap.set("n", lhs, function()
          M.toggle(buf)
        end, { desc = "Auto Format (" .. (buf and "Buffer" or "Global") .. ")" })
        return self
      end
      ...
      return Snacks.toggle({
        ...
        get = function()
          ...
          return Util.lazy.format.enabled()
        end,
        set = function(state)
          Util.lazy.format.enable(state, buf)
        end,
      }):map(lhs)
    end
  }
