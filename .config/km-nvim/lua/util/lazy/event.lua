local Config = require("util.lazy.core.config")
...
-- Can be used by distros to add custom events (see usage in Util.lazy)
...
function M:_add(event)
  ...
  vim.api.nvim_create_autocmd(event.event, {
    ...
    callback = function(ev)
      if done or not self.active[event.id] then return end
      -- HACK: work-around for https://github.com/neovim/neovim/issues/25526
      done = true
      if event.id ~= "VeryLazy" then Util.track({ [self.type] = event.id }) end
      ...
      if event.id ~= "VeryLazy" then Util.track() end
    end,
  })
end
...
function M.get_augroups(event)
  ...
  for _, autocmd in ipairs(vim.api.nvim_get_autocmds({ event = event })) do
    if autocmd.group_name then table.insert(groups, autocmd.group_name) end
  end
  return groups
end
...
function M.trigger(opts)
  if opts.group or opts.exclude == nil then return M._trigger(opts) end
