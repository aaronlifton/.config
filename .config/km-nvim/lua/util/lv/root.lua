...
function M.detectors.lsp(buf)
  ...
  local clients = Util.lazy.lsp.get_clients({ bufnr = buf })
...
function M.info()
  ...
  lines[#lines + 1] = "```"
  Util.lazy.info(lines, { title = "Util.lazy Roots" })
...
function M.setup()
  vim.api.nvim_create_user_command("LazyRoot", function()
    Util.lazy.root.info()
  end, { desc = "Util.lazy roots for the current buffer" })
...
function M.get(opts)
  ...
  return Util.lazy.is_win() and ret:gsub("/", "\\") or ret
end
