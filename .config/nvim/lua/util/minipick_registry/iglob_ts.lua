...
-- Configuration:
--   vim.g.minipick_iglob_ts_highlight = true/false (default: true)
--   vim.g.minipick_iglob_ts_max_items = number (default: 200)
H.grep_ts_show = function(buf_id, items, query, ts_highlight_override)
  local sep = string.char(31)
  local sep_pattern = vim.pesc(sep)
  ...
  if ts_enabled == nil then ts_enabled = vim.g.minipick_iglob_ts_highlight end
  ...
  local max_ts_items = vim.g.minipick_iglob_ts_max_items or 200
...
local function create_iglob_picker(MiniPick)
  return function(local_opts, opts)
    local function igrep_live(local_opts, opts)
      ...
      -- TS highlighting state (nil = use global setting)
      ...
      local function build_name_suffix()
        ...
        if ts_highlight == false then flag_parts[#flag_parts + 1] = "no-ts" end
      ...
      local function get_show_func()
        return function(buf_id, items, query)
          H.grep_ts_show(buf_id, items, query, ts_highlight)
        end
      end

      local name = build_name()
      local default_source =
        { name = name, show = get_show_func(), choose_marked = require("util.minipick_registry.trouble").trouble_choose_marked }
      ...
      local function toggle_dotall()
        ...
        if flags.extra.dotall then
          ...
          if not query_str:find(vim.pesc(dotall_pattern), 1, true) then
            query_str = query_str .. dotall_pattern
          end
        else
          -- Remove (?s:.) from query
