...
local live_grep_opts = {
  ...
  actions = {
    ...
    ["alt-i"] = toggle_flag("--iglob=!{test,spec}/"),
...
return {
  ...
  { import = "plugins.extras.telescope.urlview" },
  {
    ...
    keys = {
      ...
      },
      {
        "<leader>sU",
        function()
          require("fzf-lua").grep_cword(vim.tbl_extend("force", live_grep_opts, { search = "", root = false }))
        end,
        desc = "Word (cwd)",
        mode = "n",
      },
      ...
      },
      {
        "<leader>sU",
        function()
          require("fzf-lua").grep_visual(vim.tbl_extend("force", live_grep_opts, { root = false }))
        end,
        desc = "Selection (cwd)",
        mode = "v",
      },
      ...
      -- | `dap_frames`           | active session jump to frame               |
      {
        "<leader>dm",
        function()
          require("fzf-lua").dap_commands({ winopts = { height = 0.33, width = 0.33 } })
        end,
        desc = "Commands",
        mode = "n",
      },
      ...
      },
      {
        "<leader>sg",
        function()
          require("fzf-lua").live_grep_glob(live_grep_opts)
        end,
        desc = "Grep (Root Dir)",
      },
      {
        "<leader>sG",
        function()
          require("fzf-lua").live_grep_glob(vim.tbl_extend("force", live_grep_opts, { root = false }))
        end,
        desc = "Grep (cwd)",
      },
      {
  ...
  },
  {
    ...
    opts = function()
      if vim.g.lsp_goto_source == "glance" and require("util").has_extra("lsp.glance") then
