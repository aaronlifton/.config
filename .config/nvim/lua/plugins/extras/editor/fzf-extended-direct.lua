    ...
    keys = {
      ...
      },
      {
        ...
        function()
          ...
          LazyVim.pick("live_grep_glob", live_grep_opts_with_reset)()
        end,
        desc = "Grep (Root Dir)",
      },
      {
      ...
      {
        "<leader><space>",
        LazyVim.pick("files", { winopts = { height = 0.33, width = 0.33, preview = { hidden = "hidden" } } }),
      ...
      },
      {
        ...
        function()
          ...
          -- local parent_dir = cwd:gsub("/[^/]*$", "")
          -- function module_root(path)
          --   local start, finish = string.find(vim.fn.expand("%:p:h"), "/node_modules/[^/]+/")
          --   if start then
