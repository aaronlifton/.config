...
def initial_window_size_func(
    opts: WindowSizeData, cached_values: Dict[str, Any]
    ...
    def initial_window_size(
        ...
        if args.edge in {"top", "bottom"}:
            ...
            window_height = int(
                cell_height * args.lines / yscale + (dpi_y / 72) * spacing + 1
            )
            ...
            window_width = int(
                cell_width * args.lines / xscale + (dpi_x / 72) * spacing + 1
            )
...
def layer_shell_config(opts: PanelCLIOptions) -> LayerShellConfig:
    ltype = (
        GLFW_LAYER_SHELL_BACKGROUND
        if opts.edge == "background"
        else GLFW_LAYER_SHELL_PANEL
    )
    edge = {
        "top": GLFW_EDGE_TOP,
    ...
    return LayerShellConfig(
        type=ltype,
        edge=edge,
        size_in_cells=max(1, opts.lines),
